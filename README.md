...
<sub>Generated by `scripts/build_docs.py` from run `d9e91966b3762858` on synthetic data (fingerprint `8cd599a2ecc5a9c3`), 2,887 out-of-sample days. Reproduce with `make study`.</sub>
<!-- RESULTS-TABLE:END -->
...
1. **It reproduces.** Same seed, same prices, bit for bit, on any machine — the generator deliberately avoids BLAS and LAPACK, whose last-bit disagreements between a laptop and a CI runner would otherwise change the data fingerprint. Downstream results are bit-identical on one machine (`etf-lab verify` proves it) and agree to the last printed digit across machines (CI checks the README against a fresh run). A claim like that is impossible against a live vendor feed, and a repo whose headline numbers the reader cannot regenerate is a screenshot.
...
Two runs sharing a `run_id` are the same experiment. If their results digests differ, **the code changed the answer** — which is either a fix or a regression, and either way it should be loud rather than silent. CI runs that check on every commit in a fresh process, which also catches anything that leaked a wall clock or an unseeded generator into a research result. Across *different* machines the digest is not expected to match — interior-point solvers and BLAS libraries disagree in the last bit — so the cross-machine check is the README one: every published number must agree with a fresh run to the precision it is printed at.
